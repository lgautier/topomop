...
The Visit duration, or 'length of stay', is defined as VISIT_END_DATE - VISIT_START_DATE. For all Visits this is <1 day, except Inpatient Visits and Non-hospital institution Visits. The CDM also contains the VISIT_DETAIL table where additional information about the Visit is stored, for example, transfers between units during an inpatient Visit.
    ...
    VISIT_END_DATE = mapped_column(
        Date,
        nullable=False,
        comment="""For inpatient visits the end date is typically the discharge date.  If a Person is still an inpatient in the hospital at the time of the data extract and does not have a visit_end_date, then set the visit_end_date to the date of the data pull. """
    )
    ...
    VISIT_END_DATETIME = mapped_column(
        DateTime,
        nullable=True,
        comment="""If a Person is still an inpatient in the hospital at the time of the data extract and does not have a visit_end_datetime, then set the visit_end_datetime to the datetime of the data pull. """
    )
    ...
    DISCHARGED_TO_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""Use this field to determine where the patient was discharged to after a visit. This concept is part of the visit domain and can indicate if a patient was transferred to another hospital or sent to a long-term care facility, for example.  It is assumed that a person is discharged to home therefore there is not a standard concept id for "home".  Use concept id = 0 when a person is discharged to home. """
    )

    # From Field CSV row 38.
    DISCHARGED_TO_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True
    )
...
class visit_detail(Base):
    ...
    VISIT_DETAIL_END_DATE = mapped_column(
        Date,
        nullable=False,
        comment="""This the end date of the patient-provider interaction.  If a Person is still an inpatient in the hospital at the time of the data extract and does not have a visit_end_date, then set the visit_end_date to the date of the data pull. """
    )
    ...
    PARENT_VISIT_DETAIL_ID = mapped_column(
        Integer,
        ForeignKey('VISIT_DETAIL.VISIT_DETAIL_ID'),
        nullable=True,
...
class procedure_occurrence(Base):
    ...
    PROCEDURE_DATE = mapped_column(
        Date,
        nullable=False,
        comment="""Use this date to determine the date the procedure started. """
    )
    ...
    PROCEDURE_END_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""Use this field to house the date that the procedure ended. """
    )

    # From Field CSV row 104.
    PROCEDURE_END_DATETIME = mapped_column(
        DateTime,
        nullable=True,
        comment="""Use this field to house the datetime that the procedure ended. """
    )
...
class device_exposure(Base):
    ...
    UNIQUE_DEVICE_ID = mapped_column(
        String(255),
        nullable=True,
        comment="""This is the Unique Device Identification (UDI-DI) number for devices regulated by the FDA, if given. """
    )

    # From Field CSV row 123.
    PRODUCTION_ID = mapped_column(
        String(255),
        nullable=True,
        comment="""This is the Production Identifier (UDI-PI) portion of the Unique Device Identification. """
    )
    ...
    UNIT_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""UNIT_SOURCE_VALUES should be mapped to a Standard Concept in the Unit domain that best represents the unit as given in the source data. """
    )
    ...
    UNIT_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""This field houses the verbatim value from the source data representing the unit of the Device. For example, blood transfusions are considered devices and can be given in mL quantities. """
    )

    # From Field CSV row 132.
    UNIT_SOURCE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""This is the concept representing the UNIT_SOURCE_VALUE and may not necessarily be standard. This field is discouraged from use in analysis because it is not required to contain Standard Concepts that are used across the OHDSI community, and should only be used when Standard Concepts do not adequately represent the source detail for the Unit necessary for a given analytic use case. Consider using UNIT_CONCEPT_ID instead to enable standardized analytics that can be consistent across the network. """
    )
...
class measurement(Base):
    ...
    MEASUREMENT_EVENT_ID = mapped_column(
        Integer,
        nullable=True,
        comment="""If the Measurement record is related to another record in the database, this field is the primary key of the linked record. """
    )

    # From Field CSV row 155.
    MEAS_EVENT_FIELD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""If the Measurement record is related to another record in the database, this field is the CONCEPT_ID that identifies which table the primary key of the linked record came from. """
    )
...
class observation(Base):
    ...
    VALUE_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""This field houses the verbatim result value of the Observation from the source data.  Do not get confused with the Observation_source_value which captures source value of the observation mapped to observation_concept_id. This field is the observation result value from the source. """
    )
...
Document Type Vocabulary classification of the note. """
    ...
    NOTE_EVENT_ID = mapped_column(
        Integer,
        nullable=True,
        comment="""If the Note record is related to another record in the database, this field is the primary key of the linked record. """
    )
    ...
    NOTE_EVENT_FIELD_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""If the Note record is related to another record in the database, this field is the CONCEPT_ID that identifies which table the primary key of the linked record came from. """
    )
...
class location(Base):
    ...
    COUNTRY_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""The Concept Id representing the country. Values should conform to the [Geography](https://athena.ohdsi.org/search-terms/terms?domain=Geography&standardConcept=Standard&page=1&pageSize=15&query=&boosts) domain. """
    )

    # From Field CSV row 243.
    COUNTRY_SOURCE_VALUE = mapped_column(
        String(80),
        nullable=True,
...
class episode(Base):
    ...
    EPISODE_ID = mapped_column(
        ...
        primary_key=True,
        nullable=False,
        comment="""A unique identifier for each Episode. """
    )
    ...
    PERSON_ID = mapped_column(
        ...
        ForeignKey('PERSON.PERSON_ID'),
        nullable=False,
        comment="""The PERSON_ID of the PERSON for whom the episode is recorded. """
    )

    # From Field CSV row 326.
    EPISODE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""The EPISODE_CONCEPT_ID represents the kind abstraction related to the disease phase, outcome or treatment. """
    )

    # From Field CSV row 327.
    EPISODE_START_DATE = mapped_column(
        Date,
        nullable=False,
        comment="""The date when the Episode beings. """
    )

    # From Field CSV row 328.
    EPISODE_START_DATETIME = mapped_column(
        DateTime,
        nullable=True,
        comment="""The date and time when the Episode begins. """
    )

    # From Field CSV row 329.
    EPISODE_END_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The date when the instance of the Episode is considered to have ended. """
    )

    # From Field CSV row 330.
    EPISODE_END_DATETIME = mapped_column(
        DateTime,
        nullable=True,
        comment="""The date when the instance of the Episode is considered to have ended. """
    )
    ...
    EPISODE_PARENT_ID = mapped_column(
        Integer,
        nullable=True,
        comment="""Use this field to find the Episode that subsumes the given Episode record. This is used in the case that an Episode are nested into each other. """
    )

    # From Field CSV row 332.
    EPISODE_NUMBER = mapped_column(
        Integer,
        nullable=True,
        comment="""For sequences of episodes, this is used to indicate the order the episodes occurred. For example, lines of treatment could be indicated here. """
    )

    # From Field CSV row 333.
    EPISODE_OBJECT_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
    ...
    EPISODE_TYPE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This field can be used to determine the provenance of the Episode record, as in whether the episode was from an EHR system, insurance claim, registry, or other sources. """
    )

    # From Field CSV row 335.
    EPISODE_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""The source code for the Episode as it appears in the source data. This code is mapped to a Standard Condition Concept in the Standardized Vocabularies and the original code is stored here for reference. """
    )

    # From Field CSV row 336.
    EPISODE_SOURCE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key to a Episode Concept that refers to the code used in the source. """
    )
...
class episode_event(Base):
    ...
    __mapper_args__ = {
        'primary_key': ('EPISODE_ID', 'EVENT_ID', 'EPISODE_EVENT_FIELD_CONCEPT_ID')
    }
    ...
    EPISODE_ID = mapped_column(
        ...
        primary_key=True,
        nullable=False,
        comment="""Use this field to link the EPISODE_EVENT record to its EPISODE. """
    )
    ...
    EVENT_ID = mapped_column(
        ...
        primary_key=True,
        nullable=False,
        comment="""This field is the primary key of the linked record in the database. For example, if the Episode Event is a Condition Occurrence, then the CONDITION_OCCURRENCE_ID of the linked record goes in this field. """
    )

    # From Field CSV row 339.
    EPISODE_EVENT_FIELD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        primary_key=True,
        nullable=False,
        comment="""This field is the CONCEPT_ID that identifies which table the primary key of the linked record came from. """
    )
...
class metadata(Base):
    ...
    METADATA_ID = mapped_column(
        ...
        primary_key=True,
        nullable=False,
        comment="""The unique key given to a Metadata record. """
    )
    ...
    VALUE_AS_NUMBER = mapped_column(
        Float,
        nullable=True,
        comment="""This is the numerical value of the result of the Metadata, if applicable and available. It is not expected that all Metadata will have numeric results, rather, this field is here to house values should they exist. """
    )
...
class cdm_source(Base):
    ...
    SOURCE_DOCUMENTATION_REFERENCE = mapped_column(
        String(255),
        nullable=True,
        comment="""Refers to a publication or web resource describing the source data, e.g. a data dictionary. """
    )
    ...
    CDM_VERSION_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""The Concept Id representing the version of the CDM. """
    )
    ...
    VOCABULARY_VERSION = mapped_column(
        String(20),
        nullable=False,
        comment="""Version of the OMOP standardised vocabularies loaded """
    )