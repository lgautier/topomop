...
Users can join the CONCEPT table with other tables in the CDM to enrich clinical data with standardized Concept information or use the CONCEPT table as a reference for mapping clinical data from source terminologies to Standard Concepts.
    ...
    DOMAIN_ID = mapped_column(
        ...
        nullable=False,
        comment="""A foreign key to the [DOMAIN](https://ohdsi.github.io/CommonDataModel/cdm531.html#domain) table the Concept belongs to. """
    )
    ...
    VOCABULARY_ID = mapped_column(
        ...
        nullable=False,
        comment="""A foreign key to the [VOCABULARY](https://ohdsi.github.io/CommonDataModel/cdm531.html#vocabulary)
...
class attribute_definition(Base):
    """Description:
    The ATTRIBUTE_DEFINITION table contains records to define each attribute
...
attributes for a given cohort within the OMOP Common Data Model.
    ...
    ATTRIBUTE_DEFINITION_ID = mapped_column(
        Integer,
        primary_key=True,
        nullable=False
    )
    ...
    ATTRIBUTE_NAME = mapped_column(
        String(255),
        nullable=False
    )

    # From Field CSV row 393.
    ATTRIBUTE_DESCRIPTION = mapped_column(
        String,
        nullable=True
    )

    # From Field CSV row 394.
    ATTRIBUTE_TYPE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('CONCEPT.CONCEPT_ID'),
        nullable=False
    )
    ...
    ATTRIBUTE_SYNTAX = mapped_column(
        String,
        nullable=True
    )