...
class person(Base):
    ...
    LOCATION_ID = mapped_column(
        ...
        nullable=True,
        comment="""The location refers to the physical address of the person. This field should capture the last known location of the person.  Any prior locations are captured in the [LOCATION_HISTORY](https://ohdsi.github.io/CommonDataModel/cdm60.html#location_history) table. """
    )
...
The Visit duration, or 'length of stay', is defined as VISIT_END_DATE - VISIT_START_DATE. For all Visits this is <1 day, except Inpatient Visits and Non-hospital institution Visits. The CDM also contains the VISIT_DETAIL table where additional information about the Visit is stored, for example, transfers between units during an inpatient Visit.
    ...
    ADMITTED_FROM_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
    ...
    ADMITTED_FROM_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True
    )
...
class device_exposure(Base):
    ...
    DEVICE_TYPE_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""You can use the TYPE_CONCEPT_ID to denote the provenance of the record, as in whether the record is from administrative claims or EHR. [Accepted Concepts](https://athena.ohdsi.org/search-terms/terms?domain=Type+Concept&standardConcept=Standard&page=1&pageSize=15&query=). """
    )
...
class measurement(Base):
    ...
    MEASUREMENT_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""The MEASUREMENT_CONCEPT_ID field is recommended for primary use in analyses, and must be used for network studies. """
    )
    ...
    UNIT_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""There is currently no recommended unit for individual measurements, i.e. it is not mandatory to represent Hemoglobin a1C measurements as a percentage. UNIT_SOURCE_VALUES should be mapped to a Standard Concept in the Unit domain that best represents the unit as given in the source data. """
    )
    ...
    MEASUREMENT_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""This field houses the verbatim value from the source data representing the Measurement that occurred. For example, this could be an ICD10 or Read code. """
    )
    ...
    UNIT_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""This field houses the verbatim value from the source data representing the unit of the Measurement that occurred. """
    )
...
class observation(Base):
    ...
    OBSERVATION_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The date of the Observation. Depending on what the Observation represents this could be the date of a lab test, the date of a survey, or the date a patient's family history was taken. """
    )
    ...
    OBSERVATION_EVENT_ID = mapped_column(
        BigInteger,
        nullable=True,
        comment="""If the Observation record is related to another record in the database, this field is the primary key of the linked record. """
    )

    # From Field CSV row 167.
    OBS_EVENT_FIELD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""If the Observation record is related to another record in the database, this field is the CONCEPT_ID that identifies which table the primary key of the linked record came from. """
    )

    # From Field CSV row 168.
    VALUE_AS_DATETIME = mapped_column(
        DateTime,
        nullable=True,
        comment="""It is possible that some Observation records might store a result as a date value. """
    )
...
class note(Base):
    ...
    NOTE_EVENT_ID = mapped_column(
        BigInteger,
        nullable=True
    )

    # From Field CSV row 179.
    NOTE_EVENT_FIELD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True
    )
...
class survey_conduct(Base):
    ...
    SURVEY_CONDUCT_ID = mapped_column(
        BigInteger,
        primary_key=True,
        nullable=False,
        comment="""Unique identifier for each completed survey. """
    )
    ...
    SURVEY_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This is the Concept that represents the survey that was completed. """
    )

    # From Field CSV row 229.
    SURVEY_START_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""Date on which the survey was started. """
    )

    # From Field CSV row 230.
    SURVEY_START_DATETIME = mapped_column(
        DateTime,
        nullable=True
    )

    # From Field CSV row 231.
    SURVEY_END_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""Date on which the survey was completed. """
    )

    # From Field CSV row 232.
    SURVEY_END_DATETIME = mapped_column(
        DateTime,
        nullable=False
    )
    ...
    PROVIDER_ID = mapped_column(
        ...
        ForeignKey('PROVIDER.PROVIDER_ID'),
        nullable=True,
        comment="""This is the Provider associated with the survey completion. """
    )

    # From Field CSV row 234.
    ASSISTED_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This is a Concept that represents whether the survey was completed with assistance or independently. """
    )

    # From Field CSV row 235.
    RESPONDENT_TYPE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This is a Concept that represents who actually recorded the answers to the survey. For example, this could be the patient or a research associate. """
    )

    # From Field CSV row 236.
    TIMING_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This is a Concept that represents the timing of the survey. For example this could be the 3-month follow-up appointment. """
    )

    # From Field CSV row 237.
    COLLECTION_METHOD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This Concept represents how the responses were collected. """
    )

    # From Field CSV row 238.
    ASSISTED_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""Source value representing whether patient required assistance to complete the survey. Example: 'Completed without assistance', 'Completed with assistance'. """
    )

    # From Field CSV row 239.
    RESPONDENT_TYPE_SOURCE_VALUE = mapped_column(
        String(100),
        nullable=True,
        comment="""Source code representing role of person who completed the survey. """
    )

    # From Field CSV row 240.
    TIMING_SOURCE_VALUE = mapped_column(
        String(100),
        nullable=True,
        comment="""Text string representing the timing of the survey. Example: Baseline, 6-month follow-up. """
    )

    # From Field CSV row 241.
    COLLECTION_METHOD_SOURCE_VALUE = mapped_column(
        String(100),
        nullable=True,
        comment="""The collection method as it appears in the source data. """
    )

    # From Field CSV row 242.
    SURVEY_SOURCE_VALUE = mapped_column(
        String(100),
        nullable=True,
        comment="""The survey name as it appears in the source data. """
    )

    # From Field CSV row 243.
    SURVEY_SOURCE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False
    )
    ...
    SURVEY_SOURCE_IDENTIFIER = mapped_column(
        String(100),
        nullable=True,
        comment="""Unique identifier for each completed survey in source system. """
    )

    # From Field CSV row 245.
    VALIDATED_SURVEY_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False
    )
    ...
    VALIDATED_SURVEY_SOURCE_VALUE = mapped_column(
        String(100),
        nullable=True,
        comment="""Source value representing the validation status of the survey. """
    )

    # From Field CSV row 247.
    SURVEY_VERSION_NUMBER = mapped_column(
        String(20),
        nullable=True,
        comment="""Version number of the questionnaire or survey used. """
    )
    ...
    VISIT_OCCURRENCE_ID = mapped_column(
        ...
        ForeignKey('VISIT_OCCURRENCE.VISIT_OCCURRENCE_ID'),
        nullable=True,
        comment="""The Visit during which the Survey occurred. """
    )

    # From Field CSV row 249.
    RESPONSE_VISIT_OCCURRENCE_ID = mapped_column(
        BigInteger,
        ForeignKey('VISIT_OCCURRENCE.VISIT_OCCURRENCE_ID'),
        nullable=True,
        comment="""The Visit during which any treatment related to the Survey was carried out. """
    )
...
class location(Base):
    ...
    LONGITUDE = mapped_column(
        Float,
        nullable=True,
        comment="""The geocoded longitude. """
    )
...
class location_history(Base):
    ...
    LOCATION_ID = mapped_column(
        ...
        ForeignKey('LOCATION.LOCATION_ID'),
        nullable=False,
        comment="""This is the LOCATION_ID for the LOCATION_HISTORY record. """
    )

    # From Field CSV row 261.
    RELATIONSHIP_TYPE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This is the relationship between the location and the entity (PERSON, PROVIDER, or CARE_SITE) """
    )
    ...
    START_DATE = mapped_column(
        Date,
        nullable=False,
        comment="""The date the relationship started """
    )
    ...
    END_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The date the relationship ended """
    )
...
class provider(Base):
    ...
    SPECIALTY_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""This is the kind of provider or specialty as it appears in the source data. This includes physician specialties such as internal medicine, emergency medicine, etc. and allied health professionals such as nurses, midwives, and pharmacists. """
    )
...
class payer_plan_period(Base):
    ...
    CONTRACT_PERSON_ID = mapped_column(
        BigInteger,
        ForeignKey('PERSON.PERSON_ID'),
        nullable=True,
        comment="""The Person who is the primary subscriber/contract owner for Plan. """
    )
    ...
    CONTRACT_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""This field represents the relationship between the PERSON_ID and CONTRACT_PERSON_ID. It should be read as PERSON_ID is the *CONTRACT_CONCEPT_ID* of the CONTRACT_PERSON_ID. So if CONTRACT_CONCEPT_ID represents the relationship 'Stepdaughter' then the Person for whom PAYER_PLAN_PERIOD record was recorded is the stepdaughter of the CONTRACT_PERSON_ID. """
    )

    # From Field CSV row 297.
    CONTRACT_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=False,
        comment="""This is the relationship of the PERSON_ID to CONTRACT_PERSON_ID as it appears in the source data. """
    )

    # From Field CSV row 298.
    CONTRACT_SOURCE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False
    )
...
Each record in the cost table account for the amount of money transacted for the clinical event. So, the COST table may be used to represent both receivables (charges) and payments (paid), each transaction type represented by its COST_CONCEPT_ID. The COST_TYPE_CONCEPT_ID field will use concepts in the Standardized Vocabularies to designate the source (provenance) of the cost data. A reference to the health plan information in the PAYER_PLAN_PERIOD table is stored in the record for information used for the adjudication system to determine the persons benefit for the clinical event.
    ...
    COST_EVENT_ID = mapped_column(
        BigInteger,
        nullable=False,
        comment="""If the Cost record is related to another record in the database, this field is the primary key of the linked record. """
    )

    # From Field CSV row 309.
    COST_EVENT_FIELD_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
        comment="""If the Cost record is related to another record in the database, this field is the CONCEPT_ID that identifies which table the primary key of the linked record came from. """
    )

    # From Field CSV row 310.
    COST_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key that refers to a Standard Cost Concept identifier in the Standardized Vocabularies belonging to the 'Cost' vocabulary. """
    )
    ...
    COST_TYPE_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key identifier to a concept in the CONCEPT table for the provenance or the source of the COST data and belonging to the 'Type Concept' vocabulary """
    )

    # From Field CSV row 312.
    COST_SOURCE_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key to a Cost Concept that refers to the code used in the source. """
    )

    # From Field CSV row 313.
    COST_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""The source value for the cost as it appears in the source data """
    )
    ...
    CURRENCY_CONCEPT_ID = mapped_column(
        Integer,
        nullable=True,
        comment="""A foreign key identifier to the concept representing the 3-letter code used to delineate international currencies, such as USD for US Dollar. These belong to the 'Currency' vocabulary """
    )
    ...
    COST = mapped_column(
        Float,
        nullable=True,
        comment="""The actual financial cost amount """
    )

    # From Field CSV row 316.
    INCURRED_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The first date of service of the clinical event corresponding to the cost as in table capturing the information (e.g. date of visit, date of procedure, date of condition, date of drug etc). """
    )

    # From Field CSV row 317.
    BILLED_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The date a bill was generated for a service or encounter """
    )

    # From Field CSV row 318.
    PAID_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The date payment was received for a service or encounter """
    )
    ...
    REVENUE_CODE_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key referring to a Standard Concept ID in the Standardized Vocabularies for Revenue codes belonging to the 'Revenue Code' vocabulary. """
    )
    ...
    DRG_CONCEPT_ID = mapped_column(
        ...
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True,
        comment="""A foreign key referring to a Standard Concept ID in the Standardized Vocabularies for DRG codes belonging to the 'DRG' vocabulary. """
    )
    ...
    REVENUE_CODE_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""The source value for the Revenue code as it appears in the source data, stored here for reference. """
    )
    ...
    DRG_SOURCE_VALUE = mapped_column(
        String(50),
        nullable=True,
        comment="""The source value for the 3-digit DRG source code as it appears in the source data, stored here for reference. """
    )

    # From Field CSV row 323.
    PAYER_PLAN_PERIOD_ID = mapped_column(
        BigInteger,
        nullable=True,
        comment="""A foreign key to the PAYER_PLAN_PERIOD table, where the details of the Payer, Plan and Family are stored. Record the payer_plan_id that relates to the payer who contributed to the paid_by_payer field. """
    )
...
class drug_era(Base):
    ...
    # From Field CSV row 327.
    DRUG_ERA_START_DATETIME = mapped_column(
        DateTime,
        nullable=False
    )

    # From Field CSV row 328.
    DRUG_ERA_END_DATETIME = mapped_column(
        DateTime,
        nullable=False
    )

    # From Field CSV row 329.
...
class dose_era(Base):
    ...
    # From Field CSV row 336.
    DOSE_ERA_START_DATETIME = mapped_column(
        DateTime,
        nullable=False,
        comment="""The date the Person started on the specific dosage, with at least 31 days since any prior exposure. """
    )

    # From Field CSV row 337.
    DOSE_ERA_END_DATETIME = mapped_column(
        DateTime,
        nullable=False
    )
...
For example, consider a Person who visits her Primary Care Physician (PCP) and who is referred to a specialist. At a later time, the Person visits the specialist, who confirms the PCP's original diagnosis and provides the appropriate treatment to resolve the condition. These two independent doctor visits should be aggregated into one Condition Era.
    ...
    # From Field CSV row 339.
    PERSON_ID = mapped_column(
        BigInteger,
        nullable=False
    )

    # From Field CSV row 340.
    CONDITION_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=False,
    ...
    # From Field CSV row 341.
    CONDITION_ERA_START_DATETIME = mapped_column(
        DateTime,
        nullable=False,
...
the condition with at least 31 days since any prior record of the same Condition. """
    ...
    # From Field CSV row 342.
    CONDITION_ERA_END_DATETIME = mapped_column(
        DateTime,
        nullable=False,
...
class metadata(Base):
    ...
    # From Field CSV row 346.
    NAME = mapped_column(
        String(250),
        nullable=False
    )

    # From Field CSV row 347.
    VALUE_AS_STRING = mapped_column(
        String(250),
        nullable=True
    )

    # From Field CSV row 348.
    VALUE_AS_CONCEPT_ID = mapped_column(
        Integer,
        ForeignKey('VOCAB.CONCEPT.CONCEPT_ID'),
        nullable=True
    ...
    # From Field CSV row 349.
    METADATA_DATE = mapped_column(
        Date,
        nullable=True
    )

    # From Field CSV row 350.
...
class cdm_source(Base):
    ...
    # From Field CSV row 355.
    SOURCE_DOCUMENTATION_REFERENCE = mapped_column(
        String(255),
        nullable=True
    )

    # From Field CSV row 356.
    CDM_ETL_REFERENCE = mapped_column(
        String(255),
        nullable=True
    )

    # From Field CSV row 357.
    SOURCE_RELEASE_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The release date of the source data. """
    )

    # From Field CSV row 358.
    CDM_RELEASE_DATE = mapped_column(
        Date,
        nullable=True,
        comment="""The release data of the CDM instance. """
    )

    # From Field CSV row 359.
